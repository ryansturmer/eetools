...
def expand_series(eia):
    scale = lambda a,b : map(lambda x : x*b, a)
    return scale(eia, 0.01) + scale(eia, 0.1) + eia + scale(eia, 10) + scale(eia, 100) + scale(eia, 1000) + scale(eia, 10000) + scale(eia, 100000)
...
class ValueList(object):
    ...
    def nearest(self, value):
        a = self.nearest_above(value)
        b = self.nearest_below(value)
        ea = abs(a-value)
        eb = abs(b-value)
...
e6  = expand_series(e6)
e12 = expand_series(e12)
e24 = expand_series(e24)
e48 = expand_series(e48)
e96 = expand_series(e96)

one_percent_resistors = ValueList(set(e96).union(set(e24)))
